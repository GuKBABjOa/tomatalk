      ...
      <div class="video-section">
        ...
        <div class="main-speaker-video">
          ...
          <div class="video-placeholder">
            <!-- <span v-if="speakerConnectionId === null">발언자가 없습니다.</span> -->
          </div>
...
const pyUrl = import.meta.env.VITE_PYTHON_URL + "/ws";
...
onMounted(() => {
  ...
  const videoElements = document.querySelectorAll<HTMLVideoElement>(".participant-video video")
  newParticipants.forEach((participant, index) => {
    if (
      videoElements[index] &&
      "addVideoElement" in participant &&
      typeof participant.addVideoElement === "function"
    ) {
...
const users = ref<{ userId: string; nickname: string; profileImage: string; position: string; order: number }[]>([])
...
const fetchDebateRoomInfo = async () => {
  try {
    ...
    if (response.status === 200) {
      ...
      const debater = users.value.find(user => user.userId === userId);
      if (debater) {
        position.value = debater.position;
      } else {
        position.value = "";
      }
    }
...
const currentSpeakerPosition = computed(() => { // 현재 발언자 입장
  ...
  const currentPosition = participants.find((p: { userId: string }) => p.userId === currentSpeakerId)
...
const nextSpeakerNickname = computed(() => {
  ...
  const nextSpeaker = participants.find((p: { userId: string }) => p.userId === nextSpeakerId);
...
const currentSpeakerNickname = computed(() => {
  ...
  const currentSpeaker = participants.find((p: { userId: string }) => p.userId === currentSpeakerId)
